...
require_relative 'SodeomSearch_types'


class SodeomSearchSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SodeomSearchHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SodeomSearchHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SodeomSearchFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SodeomSearchUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SodeomSearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SodeomSearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SodeomSearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SodeomSearchError.new(
        "#{op}_allow",
        "SodeomSearchSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SodeomSearchHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SodeomSearchError.new(
        "graphql_error", "SodeomSearchSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SodeomSearchSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
