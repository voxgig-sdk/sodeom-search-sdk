...
class SearchEntityTest < Minitest::Test
  def test_create_instance
    testsdk = SodeomSearchSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = SodeomSearchSDK.test(seed, nil)
    seen = base.Search(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = SodeomSearchConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = SodeomSearchSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SODEOM_SEARCH_TEST_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_basic_setup(extra)
  ...
  client = SodeomSearchSDK.test(options, extra)
  ...
  entid_env_raw = ENV["SODEOM_SEARCH_TEST_SEARCH_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["SODEOM_SEARCH_TEST_SEARCH_ENTID"])
  if idmap_resolved.nil?
    idmap_resolved = Helpers.to_map(idmap)
  end

  if env["SODEOM_SEARCH_TEST_LIVE"] == "TRUE"
    merged_opts = Vs.merge([
      {
      },
      extra || {},
    ])
    client = SodeomSearchSDK.new(Helpers.to_map(merged_opts))
  end

  live = env["SODEOM_SEARCH_TEST_LIVE"] == "TRUE"
  {
    client: client,
    data: entity_data,
    idmap: idmap_resolved,
    env: env,
    explain: env["SODEOM_SEARCH_TEST_EXPLAIN"] == "TRUE",
    live: live,
