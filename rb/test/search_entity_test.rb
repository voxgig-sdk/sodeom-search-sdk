...
class SearchEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SODEOMSEARCH_TEST_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_basic_setup(extra)
  ...
  entid_env_raw = ENV["SODEOMSEARCH_TEST_SEARCH_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["SODEOMSEARCH_TEST_SEARCH_ENTID"])
  ...
  live = env["SODEOMSEARCH_TEST_LIVE"] == "TRUE"
  {
    ...
    env: env,
    explain: env["SODEOMSEARCH_TEST_EXPLAIN"] == "TRUE",
    live: live,
