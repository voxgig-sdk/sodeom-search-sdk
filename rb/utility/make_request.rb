...
module SodeomSearchUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SodeomSearchResponse.new({})
    ...
    if fetch_err
      ...
      response = SodeomSearchResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SodeomSearchResponse.new(fetched)
    else
