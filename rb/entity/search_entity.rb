# SodeomSearch SDK Search entity
...
class SearchEntity
  ...
  def data_set(args)
    if args
      @_data = SodeomSearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = SodeomSearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Search>, Array] the matching Search items; raises SodeomSearchError on failure
