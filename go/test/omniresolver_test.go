...
import (
	...
	omni "github.com/voxgig-sdk/sodeom-search-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SodeomSearchError); is {
				out := map[string]any{
					"name":    "SodeomSearchError",
...
func omniCtx(arg any, client *sdk.SodeomSearchSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SodeomSearchSDK); is {
			client = live
