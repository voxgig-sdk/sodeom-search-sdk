...
//     (\`new SodeomSearchSDK(...)\` / \`SodeomSearchSDK.test(...)\` become
//     \`SodeomSearchSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SodeomSearchSDK'
...
// rewrite real client construction (\`new SodeomSearchSDK(...)\` and any
// \`SodeomSearchSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SodeomSearchSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SodeomSearchSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
