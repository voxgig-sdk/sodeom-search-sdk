...
class SearchEntity
{
    ...
    {
        if ($args) {
            $this->_data = SodeomSearchHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = SodeomSearchHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws SodeomSearchError on failure (item-5 convention).
     */
