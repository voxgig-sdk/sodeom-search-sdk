...
// SodeomSearch SDK retry feature
...
class SodeomSearchRetryFeature extends SodeomSearchBaseFeature
{
    ...
    public function init(SodeomSearchContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SodeomSearchContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SodeomSearchContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SodeomSearchContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
