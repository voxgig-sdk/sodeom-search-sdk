...
class FtCtrl extends SodeomSearchControl
{
...
class FtClient
{
    ...
    private SodeomSearchUtility $utility;

    public function __construct(SodeomSearchUtility $utility, array $options)
    {
    ...
    public function get_utility(): SodeomSearchUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SodeomSearchContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SodeomSearchContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SodeomSearchContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SodeomSearchConfig::make_feature($name);
    ...
    private static function buildUrl(SodeomSearchSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SodeomSearchError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SodeomSearchResponse($fetched);
        }
    ...
    private function fail(SodeomSearchContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SodeomSearchContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
