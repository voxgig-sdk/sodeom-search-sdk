...
// SodeomSearch SDK pipeline test
//
...
{
    private static function utility(): SodeomSearchUtility
    {
        return new SodeomSearchUtility();
    }
    ...
    private static function ctx(array $over = []): SodeomSearchContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SodeomSearchContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SodeomSearchOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SodeomSearchError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SodeomSearchOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SodeomSearchMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SodeomSearchOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SodeomSearchMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SodeomSearchOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SodeomSearchMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SodeomSearchSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SodeomSearchMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SodeomSearchResponse([]),
            'result' => new SodeomSearchResult([]),
        ]);
        [, $err] = SodeomSearchMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new SodeomSearchSpec([]),
            'response' => null,
            'result' => new SodeomSearchResult([]),
        ]);
        [, $err] = SodeomSearchMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SodeomSearchSpec(['step' => 's']),
            'response' => new SodeomSearchResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SodeomSearchResult([]),
        ]);
        [, $err] = SodeomSearchMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SodeomSearchSpec(['step' => 's']),
            'response' => new SodeomSearchResponse(self::resp(200, ['v' => 1])),
            'result' => new SodeomSearchResult([]),
        ]);
        [, $err] = SodeomSearchMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SodeomSearchSpec(['step' => 's']),
            'response' => new SodeomSearchResponse(self::resp(200, ['v' => 2])),
            'result' => new SodeomSearchResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SodeomSearchMakeResponse::call($ctx);
    ...
    {
        $preset = new SodeomSearchResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SodeomSearchSpec([]),
            'response' => new SodeomSearchResponse([]),
            'result' => new SodeomSearchResult([]),
        ...
        [$got, $err] = SodeomSearchMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SodeomSearchResult([])]);
        [, $err] = SodeomSearchMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SodeomSearchSpec([]), 'result' => null]);
        [, $err] = SodeomSearchMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SodeomSearchSpec(['step' => 's']),
            'result' => new SodeomSearchResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SodeomSearchOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SodeomSearchMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SodeomSearchSpec(['step' => 's']),
            'result' => new SodeomSearchResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SodeomSearchOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SodeomSearchMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SodeomSearchResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SodeomSearchSpec([]),
            'result' => new SodeomSearchResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SodeomSearchMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SodeomSearchMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new SodeomSearchError('boom', 'boom');
        $utility->fetcher = function (SodeomSearchContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SodeomSearchSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SodeomSearchMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SodeomSearchContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SodeomSearchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SodeomSearchResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        SodeomSearchMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SodeomSearchContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SodeomSearchSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SodeomSearchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SodeomSearchResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SodeomSearchSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SodeomSearchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SodeomSearchMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SodeomSearchMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SodeomSearchResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SodeomSearchDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SodeomSearchResult(['ok' => false])]);
        $this->expectException(SodeomSearchError::class);
        SodeomSearchDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SodeomSearchResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SodeomSearchMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        SodeomSearchMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SodeomSearchBaseFeature();
        $b = new SodeomSearchBaseFeature();
        SodeomSearchFeatureAdd::call($ctx, $a);
        SodeomSearchFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SodeomSearchBaseFeature
    {
        $f = new SodeomSearchBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        SodeomSearchFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): SodeomSearchContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SodeomSearchSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SodeomSearchPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        SodeomSearchPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new SodeomSearchResponse(['headers' => null]),
            'result' => new SodeomSearchResult([]),
        ]);
        SodeomSearchResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        SodeomSearchResultBody::call($ctx);
