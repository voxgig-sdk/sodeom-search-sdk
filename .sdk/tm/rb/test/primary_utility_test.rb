...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = SodeomSearchOmni.make_runner(TEST_JSON_FILE, SodeomSearchSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    SodeomSearchError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : SodeomSearchOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = SodeomSearchOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
