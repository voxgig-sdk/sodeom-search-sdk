...
class PipelineTest < Minitest::Test
  ...
  def spec_of(map = {})
    SodeomSearchSpec.new({ "step" => "s", "method" => "GET", "headers" => {} }.merge(map))
  end
  ...
  def test_make_response_guards_missing_spec_response_result
    ...
    ctx.result = SodeomSearchResult.new({})
  ...
  def test_make_result_empty_list_yields_an_empty_resdata_array
    ...
    ctx.spec = spec_of
    ctx.result = SodeomSearchResult.new({ "ok" => true, "resdata" => [] })
    result, err = @utility.make_result.call(ctx)
    assert_nil err
  ...
  def test_make_result_short_circuits_a_preset_result
    ctx = make_ctx
    preset = SodeomSearchResult.new({ "ok" => true })
  ...
  def test_make_request_a_transport_error_tuple_lands_on_the_response
    boom = SodeomSearchError.new("boom", "boom")
  ...
  def test_make_request_a_fetchdef_error_surfaces_as_a_response_error
    u = @client.get_utility
    u.make_fetch_def = ->(ctx) { [nil, SodeomSearchError.new("fetchdef_boom", "boom")] }
    ctx = make_ctx(utility: u)
    ctx.spec = request_spec
    response, err = u.make_request.call(ctx)
    assert_nil err
  ...
  def test_done_returns_resdata_on_success
    ctx = make_ctx
    ctx.result = SodeomSearchResult.new({ "ok" => true, "resdata" => 42 })
    assert_equal 42, @utility.done.call(ctx)
  end

  def test_done_raises_the_error_when_not_ok
    ctx = make_ctx
    ctx.result = SodeomSearchResult.new({ "ok" => false })
    assert_raises(SodeomSearchError) { @utility.done.call(ctx) }
  end

  def test_done_cleans_ctrl_explain_on_success
    ctx = make_ctx
    ctx.ctrl.explain = { "result" => { "err" => "x" } }
    ctx.result = SodeomSearchResult.new({ "ok" => true, "resdata" => 7 })
  ...
  def test_make_error_returns_resdata_when_throw_is_disabled
    ...
    ctx.result = SodeomSearchResult.new({ "ok" => false, "resdata" => "fallback" })
  ...
  def test_make_error_preserves_the_error_code
    ctx = make_ctx
    err = assert_raises(SodeomSearchError) {
  ...
  def test_feature_add_appends_in_call_order
    ...
    client.features = []
    a = SodeomSearchBaseFeature.new
    b = SodeomSearchBaseFeature.new
  ...
  def auth_ctx(options, headers)
    ctx = SodeomSearchContext.new({
      "client" => OptClient.new(options),
      "utility" => @utility,
      "opname" => "load",
    }, nil)
    ctx.spec = headers.nil? ? nil : SodeomSearchSpec.new({ "headers" => headers, "step" => "s" })
    ctx
  end
  ...
  def test_result_headers_with_non_hash_headers_yields_an_empty_map
    ctx = make_ctx
    ctx.response = SodeomSearchResponse.new({ "status" => 200 })
    ctx.result = SodeomSearchResult.new({})
  ...
  def test_result_body_skips_parsing_when_the_body_is_absent
    ctx = make_ctx
    ctx.response = SodeomSearchResponse.new({
