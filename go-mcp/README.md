...
[MCP](https://modelcontextprotocol.io) server exposing the SodeomSearch SDK as
two agent tools — `sodeom-search_list` and `sodeom-search_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sodeom-search-mcp)
make build

# 2. Provide credentials via the environment
export SODEOM_SEARCH_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sodeom-search \
  -- /absolute/path/to/sodeom-search-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sodeom-search-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sodeom-search_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// sodeom-search_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sodeom-search-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sodeom-search \
     -- "$PWD"/dist/*/sodeom-search-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sodeom-search_list` and `sodeom-search_load` tools now appear
   in new sessions. Ask the agent to *"list search using sodeom-search"*
   and it calls `sodeom-search_list` with `{"entity":"search"}`.
...
```sh
export SODEOM_SEARCH_APIKEY=sk_live_xxx            # API key
export SODEOM_SEARCH_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `sodeom-search_list` tool
...
### Call the `sodeom-search_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `sodeom-search_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sodeom-search_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SODEOM_SEARCH_APIKEY` | API key sent with every request. |
| `SODEOM_SEARCH_BASE` | Optional override of the API base URL. |
...
```sh
./sodeom-search-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sodeom-search_load","arguments":{"entity":"search","query":{"id":1}}}}'
```
