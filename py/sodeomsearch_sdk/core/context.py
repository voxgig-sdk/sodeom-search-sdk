...
from sodeomsearch_sdk.core.helpers import get_ctx_prop, to_map
...
class SodeomSearchContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, SodeomSearchSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, SodeomSearchResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, SodeomSearchResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return SodeomSearchError(code, msg, self)
