...
class TestSearchEntity:
    ...
    def test_should_stream(self):
        ...
        base = SodeomSearchSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SODEOM_SEARCH_TEST_SEARCH_ENTID JSON to run live")
        client = setup["client"]
...
def _search_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SODEOM_SEARCH_TEST_SEARCH_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("SODEOM_SEARCH_TEST_LIVE") == "TRUE":
        ...
        client = SodeomSearchSDK(helpers.to_map(merged_opts))

    _live = env.get("SODEOM_SEARCH_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("SODEOM_SEARCH_TEST_EXPLAIN") == "TRUE",
        "live": _live,
