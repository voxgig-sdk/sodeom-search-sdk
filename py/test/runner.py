...
class SodeomSearchTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SodeomSearchTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SodeomSearchTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SodeomSearchTestRunner.getenv("SODEOM_SEARCH_TEST_LIVE")
        override = SodeomSearchTestRunner.getenv("SODEOM_SEARCH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SodeomSearchTestRunner.getenv(key)
        ...
        explain = SodeomSearchTestRunner.getenv("SODEOM_SEARCH_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SodeomSearchTestRunner._test_control is not None:
            return SodeomSearchTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SodeomSearchTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SodeomSearchTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SodeomSearchTestRunner.load_test_control()
...
def load_env_local():
    SodeomSearchTestRunner.load_env_local()


def env_override(m):
    return SodeomSearchTestRunner.env_override(m)


def entity_data(v):
    return SodeomSearchTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SodeomSearchTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SodeomSearchTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SodeomSearchTestRunner.load_test_control()


def live_client_options():
    return SodeomSearchTestRunner.live_client_options()


def live_delay_ms():
    return SodeomSearchTestRunner.live_delay_ms()
