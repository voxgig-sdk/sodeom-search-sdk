...
The Sodeom Search API enables users to perform private searches without any tracking, returning results in a structured JSON format.

Learn more about Sodeom Search API at [sodeom.com](https://sodeom.com).
...
const items = await client.Search().list({ q: "example" })
```
