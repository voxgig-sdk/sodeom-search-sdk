// Command SodeomSearch-cli is a CLI + REPL for the SodeomSearch SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SODEOM_SEARCH_APIKEY (api key), SODEOM_SEARCH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SODEOM_SEARCH_APIKEY carries the API key and
	// SODEOM_SEARCH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SODEOM_SEARCH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SODEOM_SEARCH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSodeomSearchSDK(opts)
