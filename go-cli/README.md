...
boru-driven command-line client **and** interactive REPL for the SodeomSearch
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sodeom-search-cli)
make build

# 2. See usage (words, entities, env vars)
./sodeom-search-cli --help

# 3. Provide credentials once, via the environment
export SODEOM_SEARCH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./sodeom-search-cli list search

# 5. Override the API base URL for a single call
SODEOM_SEARCH_BASE=https://api.example.com ./sodeom-search-cli list search

# 6. No arguments -> interactive REPL
./sodeom-search-cli
sodeom-search> list search
sodeom-search> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sodeom-search-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/sodeom-search-cli list search
   ```
...
Run with no arguments to open a REPL (prompt `sodeom-search>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sodeom-search-cli`. |
...
(`./sodeom-search-cli <expr>`) and interactively (the REPL), and expressions compose the
